import 'package:CMRS/Controller/admin.dart';
...
class CartServices {
  ...
  registerItem({required String name, String? description,String? quantity, String? category,int? price,String? itemId,required String? itemPic}) async {
    ...
    try {
      await firestore.collection('users').doc(userCntr.user!.value.uid).collection("cart-items").doc(itemId).set(x.toJson()).then((value) => snackbar('Done', 'Successfully Added Item to Cart'));
      loading(false);
    } catch (e) {
      loading(false);
  ...
  // }
  Stream<List<ItemModel>>? streamAllCartItems() {
    try {
      return firestore.collection('users').doc(userCntr.user!.value.uid).collection("cart-items").snapshots().map((event) {
        loading(false);
        List<ItemModel> list = [];
  ...
  update(ItemModel item,String index,String Value) async {
    try {
      loading(true);
      print(Value);
      await firestore.collection('users').doc(userCntr.user!.value.uid).collection("cart-items")
          .doc(item.itemId)
